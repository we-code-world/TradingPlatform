...
<head>
    ...
    <script type="text/javascript">
        ...
        function doEdit(id){
            $.ajax({
                url:'${pageContext.request.contextPath}/Admin/getWTB',
                data:{"id":id},
                success:function(json){
                    if(json.result.toString()==="ok"){
                        $('#myeditform').find("input[name='userId']").val(json.wtb.userid);
                        $('#myeditform').find("input[name='goodsname']").val(json.wtb.seekname);
                        $('#myeditform').find("input[name='wtbprice']").val(json.wtb.wprice);
                        $('#myeditform').find("input[name='wtbinfo']").val(json.wtb.wtbdata);
        ...
        function doSavePass(){
            $.ajax({
                url:'${pageContext.request.contextPath}/Admin/updateWTBpass',
                type:'POST',
                data:{"id":$('#pass').val()},
                success:function(json){
                    if(json.result==="ok"){
        ...
        function doSaveRefuse(){
            $.ajax({
                url:'${pageContext.request.contextPath}/Admin/updateWTBRefuse',
                type:'POST',
                data:{"id":$('#refuse').val()},
                success:function(json){
                    if(json.result==="ok"){
...
<div id="content" style="margin-right: 100px; margin-top: 40px;">
    ...
    <div class="container" style="width: 900px;">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
            <c:forEach var="wtb" items="${requestScope.get('wtbs').list}" >
                <tr>
                    <td>${wtb.getUserid()}</td>

                    <td>
                        <c:if test="${wtb.getAdministrator_agree()==0}"><span class="btn-primary btn-warning">尚待审核</span> </c:if>
                        <c:if test="${wtb.getAdministrator_agree()==1}"><span class="btn-primary btn-danger">审核未通过</span> </c:if>
                        <c:if test="${wtb.getAdministrator_agree()==2}"><span class="btn-primary  btn-success">审核已通过</span></c:if>
                    </td>
                    <td>


                        <c:if test="${wtb.getAdministrator_agree()!=0}"><button type="button" class="btn btn-success" >无需审核</button></c:if>

                        <c:if test="${wtb.getAdministrator_agree()==0}"><button type="button" class="btn btn-info" onclick="doEdit(${wtb.getSeekid()})">立即审核</button></c:if>
        ...
        <div style="text-align: right">
            <div class="pagination">
                <ul>
                    <c:if test="${requestScope.get('wtbs').pageNum != 1 }">
                        <li><a>第1页</a></li>
                    </c:if>
                    <c:if test="${requestScope.get('wtbs').isFirstPage = false }">
                        <li><a href="${pageContext.request.contextPath}/Admin/pagewtb?id=${requestScope.get('admin').getUserid()}&&pageNum=${requestScope.get('wtbs').pageNum-1 }&&pageSize=10">上一页</a>
                        </li>
                    </c:if>
                    <li>第${requestScope.get('wtbs').pageNum}页</li>
                    <c:if test="${requestScope.get('wtbs').isLastPage = false}">
                        <li><a href="${pageContext.request.contextPath}/Admin/pagewtb?id=${requestScope.get('admin').getUserid()}&&pageNum=${requestScope.get('wtbs').pageNum+1 }&&pageSize=10">下一页</a>
                        </li>
                    </c:if>
                </ul>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form-horizontal" id="myeditform" name="myform">
                    ...
                    <div class="form-group">
                        <label class="col-sm-4 control-label">求购用户ID:</label>
                        <div class="col-sm-8">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <input type="text" readonly class="form-control" name="wtbprice" style="margin-top: 8px;" />
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-4 control-label">求购信息:</label>
                        <div class="col-sm-8">
                            <input type="text" readonly class="form-control" name="wtbinfo" style="margin-top: 8px;" />
                        </div>
                    </div>
                </form>
            </div>
